...
int main(void) {
        ...
        switch (axis) {
            ...
            case 'r':
                ...
                if(counterInCurrentDirection == beforeBorder){
                    ...
                    //  Элемент не у границы. Происходит подготовка к заполнению
                    //  в текущую сторону
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int j = 0; j < N; j++) {
            ...
            //  Если число имеет 1 знак, то справа делается дополнительный отступ
            (total[i][j] < 10) ? printf("%d  ", total[i][j]) : printf("%d ", total[i][j]);
        }
